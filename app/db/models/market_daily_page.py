...
from sqlalchemy import Date, DateTime, Enum, ForeignKey, Integer, Numeric, SmallInteger, Text, UniqueConstraint
...
class MarketDailyPage(Base):
    ...
    status: Mapped[PageStatus] = mapped_column(Enum(PageStatus, name="page_status_enum"))
...
class MarketDailyPageMarket(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    page_id: Mapped[int] = mapped_column(ForeignKey("stock.market_daily_page.id", ondelete="CASCADE"))
...
class MarketDailyPageMarketCluster(Base):
    ...
    representative_published_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class MarketDailyPageArticleLink(Base):
    ...
    page_market: Mapped[MarketDailyPageMarket] = relationship(back_populates="article_links")
