...
class BuildPageSnapshotStep(BatchStep):
    ...
    async def run(
        ...
        cluster_article_links = await cluster_repo.list_cluster_article_links_by_business_date(
        ...
        for summary in summaries:
            summary_by_type[(summary.summary_type, summary.market_type, summary.cluster_id)] = summary

        version_no = await snapshot_repo.get_next_version_no(context.business_date)
        page_status = PageStatus.PARTIAL.value if context.partial_reasons or context.warning_messages else PageStatus.READY.value
        global_headline_summary = summary_by_type.get((AiSummaryType.GLOBAL_HEADLINE.value, None, None))
        ...
        for article_link in cluster_article_links:
            article_links_by_market.setdefault(article_link["market_type"], []).append(article_link)
        ...
        for display_order, market_type in enumerate(["US", "KR"], start=1):
            market_summary = summary_by_type.get((AiSummaryType.MARKET_SUMMARY.value, market_type, None))
            market_metadata = getattr(market_summary, "metadata_json", {}) or {}
            page_market_id = await snapshot_repo.create_page_market(
                page_id=page_id,
                market_type=market_type,
                display_order=display_order,
                market_label="미국 증시 일간 요약" if market_type == "US" else "한국 증시 일간 요약",
            ...
            for index_order, index in enumerate(indices_by_market.get(market_type, []), start=1):
            ...
            for cluster_order, cluster in enumerate(by_market.get(market_type, []), start=1):
                card_summary = summary_by_type.get(
                    (AiSummaryType.CLUSTER_CARD_SUMMARY.value, market_type, cluster["id"])
                )
                await snapshot_repo.insert_page_market_cluster(
                    {
                        ...
                        "title": cluster["title"],
                        "summary": getattr(card_summary, "body", None) or cluster["summary_short"],
                        ...
                        "representative_publisher_name": cluster.get("representative_publisher_name"),
                        "representative_published_at": cluster.get("representative_published_at"),
                        "representative_origin_link": cluster.get("representative_origin_link"),
                        "representative_naver_link": cluster.get("representative_naver_link"),
                    }
                )
            for link_order, article_link in enumerate(article_links_by_market.get(market_type, []), start=1):
        ...
        if page_status == PageStatus.PARTIAL.value:
            await repository.add_event(
                ...
                context_json={"warnings": context.warning_messages, "partialReasons": context.partial_reasons},
            )
        context.log_messages.append(f"Built page snapshot pageId={page_id}, versionNo={version_no}.")
        return context
