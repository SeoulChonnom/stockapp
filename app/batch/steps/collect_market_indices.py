...
from app.batch.providers.market_index_provider import MarketIndexProvider, YFINANCE_PROVIDER_NAME
...
class CollectMarketIndicesStep(BatchStep):
    ...
    async def run(
        ...
        if context.rebuild_page_only:
            context.log_messages.append("Skipped market index collection because rebuild_page_only=true.")
            return context
        ...
        for result in results:
            ...
            if result.source_date != context.business_date:
                context.warning_messages.append(
                    f"{result.market_type}:{result.index_code} used fallback trading date {result.source_date.isoformat()}."
                )
