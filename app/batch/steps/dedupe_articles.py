...
from app.batch.normalizers import build_dedupe_hash, canonicalize_link, excerpt_text, normalize_title
...
class DedupeArticlesStep(BatchStep):
    ...
    async def run(
        ...
        if context.rebuild_page_only:
            context.log_messages.append("Skipped dedupe because rebuild_page_only=true.")
            return context
        ...
        raw_articles = await raw_repo.list_articles_by_business_date(context.business_date)
        if not raw_articles:
            ...
            context.log_messages.append("No raw articles were available for deduplication.")
            return context
        ...
        context.log_messages.append(
            f"Deduplicated {len(raw_articles)} raw articles into {len(processed_ids)} processed articles."
        )
        await repository.add_event(
            job_id=context.job_id,
            step_code=self.step_code,
