...
class ClustersService:
    ...
    async def get_cluster_detail(self, cluster_id: str) -> ClusterDetailResponse:
        ...
        if cluster is None:
            raise NotFoundError("CLUSTER_NOT_FOUND", "요청한 뉴스 클러스터를 찾을 수 없습니다.")
        ...
        ordered_articles = [by_id[row["processed_article_id"]] for row in cluster_articles if row["processed_article_id"] in by_id]
        return build_cluster_detail_response(cluster, representative_article, ordered_articles)
