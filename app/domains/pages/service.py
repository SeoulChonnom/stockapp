...
class PagesService:
    ...
    async def get_latest_page(self) -> DailyPageResponse:
        ...
        if page is None:
            raise NotFoundError("LATEST_PAGE_NOT_FOUND", "가장 최근 생성된 페이지가 존재하지 않습니다.")
    ...
    async def get_page_by_date(
        ...
        page = await self._repo.get_page_header_by_business_date(business_date, version_no)
        if page is None:
            raise NotFoundError("PAGE_NOT_FOUND", "요청한 날짜의 페이지가 존재하지 않습니다.")
        return await self._build_page(page)
    ...
    async def _build_page(self, page: dict) -> DailyPageResponse:
        ...
        return build_daily_page_response(page, markets, indices, clusters, article_links)
