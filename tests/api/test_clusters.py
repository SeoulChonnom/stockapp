...
def test_get_cluster_detail_returns_contract(client, sample_cluster_detail_payload):
    response = client.get(f"/stock/api/news/clusters/{sample_cluster_detail_payload['clusterId']}")
...
def test_get_cluster_detail_returns_404_when_missing(client):
    response = client.get("/stock/api/news/clusters/7b9845f6-5c3d-4f2c-a81d-8dcb0b5dd6d2")
