...
@pytest.mark.asyncio
async def test_get_current_user_accepts_valid_user_token(jwt_access_token: str, jwt_subject: str):
...
@pytest.mark.asyncio
async def test_get_current_user_accepts_token_without_username_claims(jwt_access_token_factory):
...
async def test_get_current_user_rejects_expired_token(jwt_access_token_factory):
    expired_token = jwt_access_token_factory(
        expires_at=datetime(2026, 3, 18, 6, 4, 59, tzinfo=timezone.utc),
    )
    with pytest.raises(auth_module.UnauthorizedError) as exc_info:
...
@pytest.mark.asyncio
async def test_get_current_user_rejects_refresh_token_as_access_token(jwt_refresh_token: str):
    with pytest.raises(auth_module.UnauthorizedError) as exc_info:
...
async def test_get_current_user_rejects_missing_or_empty_roles(jwt_access_token_factory, roles):
...
async def test_get_current_user_rejects_missing_config(jwt_access_token_factory, monkeypatch, secret_value):
...
@pytest.mark.asyncio
async def test_get_current_user_rejects_invalid_algorithm_config(jwt_access_token_factory, monkeypatch):
