...
GENERATED_AT = datetime(2026, 3, 18, 6, 12, 10, tzinfo=UTC)
UPDATED_AT = datetime(2026, 3, 18, 6, 20, 0, tzinfo=UTC)
...
JWT_TEST_SECRET = (
    base64.urlsafe_b64encode(JWT_TEST_SECRET_BYTES).decode('ascii').rstrip('=')
)
JWT_ALTERNATE_TEST_SECRET_BYTES = (
    b'stockapp-sibling-jwt-contract-alternate-secret-material-for-hs512-'
    b'tests-2026-04-23'
)
JWT_ALTERNATE_TEST_SECRET = (
    base64.urlsafe_b64encode(JWT_ALTERNATE_TEST_SECRET_BYTES)
    .decode('ascii')
    .rstrip('=')
)
JWT_ISSUER = 'slcnapp'
...
JWT_ISSUED_AT = datetime(2026, 3, 18, 6, 0, 0, tzinfo=UTC)
JWT_VALIDATED_AT = datetime(2026, 3, 18, 6, 5, 0, tzinfo=UTC)
...
def mint_test_jwt(
    *,
    ...
    resolved_audience: str | list[str] = (
        audience if audience is not None else [JWT_AUDIENCE]
    )
...
def validate_test_access_token(
    ...
    *,
    ...
    expected_signature = hmac.new(
        resolved_secret, signing_input, hashlib.sha512
    ).digest()
    ...
    return {
        ...
        'markets': [
            ...
            },
            {
                ...
                'summaryBody': (
                    '외국인 순매수와 반도체 업황 기대가 지수 반등을 이끌었다.'
                ),
                'analysis': {
                    'background': ['외국인 순매수', '대형 수출주 회복'],
                    'keyThemes': ['반도체', '자동차', '원화'],
                    'outlook': (
                        '환율 안정 여부와 미국 기술주 흐름이 다음 세션의 관건이다.'
                    ),
                },
                'indices': [
                    {
...
def build_archive_item_payload(
    page_id: int = 501, business_date: str = '2026-03-17'
) -> dict[str, Any]:
    return {
        'pageId': page_id,
...
def build_batch_run_payload() -> dict[str, Any]:
    return {
        ...
        'queuedAt': '2026-03-18T06:10:00+00:00',
    }
        ...
        'items': [
            {
                ...
                'sourceJobId': None,
                'sourcePageId': None,
                'queuedAt': '2026-03-18T06:09:58+00:00',
                'attemptCount': 1,
                'maxAttempts': 3,
