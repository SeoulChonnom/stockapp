...
@pytest.mark.anyio
async def test_get_latest_page_header_orders_by_latest_business_date_then_version(sample_page_snapshot_row):
...
@pytest.mark.anyio
async def test_get_page_header_by_business_date_uses_explicit_version_when_provided(sample_page_snapshot_row):
    session = RecordingAsyncSession(results=[DummyResult([sample_page_snapshot_row])])
    repo = PageSnapshotRepository(session)

    result = await repo.get_page_header_by_business_date(sample_page_snapshot_row["business_date"], version_no=3)
...
@pytest.mark.anyio
async def test_list_archive_page_headers_prefers_latest_version_per_day(sample_archive_list_payload):
    session = RecordingAsyncSession(results=[DummyResult(sample_archive_list_payload["items"])])
    repo = PageSnapshotRepository(session)
