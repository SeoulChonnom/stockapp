...
async def test_get_cluster_articles_orders_by_article_rank(sample_cluster_article_rows):
    ...
    result = await repo.get_cluster_articles(sample_cluster_article_rows[0]["cluster_id"])
...
@pytest.mark.anyio
async def test_get_processed_articles_returns_requested_rows_in_order(sample_processed_article_rows):
    session = RecordingAsyncSession(results=[DummyResult(sample_processed_article_rows)])
    repo = ClusterRepository(session)

    result = await repo.get_processed_articles([row["id"] for row in sample_processed_article_rows])
