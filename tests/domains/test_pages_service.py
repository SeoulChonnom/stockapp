...
class FakePageSnapshotRepository:
    def __init__(self, *, page_header, markets, indices, clusters, article_links, archive_items, archive_total_count):
    ...
    async def get_page_header_by_business_date(self, business_date, version_no=None):
        self.calls.append(("get_page_header_by_business_date", business_date, version_no))
        return self.page_header
...
@pytest.fixture
def page_repository(sample_page_snapshot_row, sample_page_market_rows, sample_page_index_rows, sample_page_cluster_rows, sample_page_article_link_rows, sample_archive_item_payload, sample_archive_list_payload):
...
@pytest.mark.anyio
async def test_pages_service_fetches_latest_page_bundle(page_repository, sample_daily_page_payload):
    ...
    assert payload["markets"][0]["topClusters"][0]["clusterId"] == sample_daily_page_payload["markets"][0]["topClusters"][0]["clusterId"]
...
@pytest.mark.anyio
async def test_pages_service_uses_versioned_lookup_when_date_is_explicit(page_repository, sample_daily_page_payload):
    service = PagesService(page_repository)
    ...
    assert page_repository.calls[0] == ("get_page_header_by_business_date", BUSINESS_DATE, 3)
...
async def test_archive_service_returns_paged_summary(page_repository):
    ...
    result = await service.list_archive(from_date=date(2026, 3, 16), to_date=date(2026, 3, 17), status="READY", page=1, size=30)
    payload = jsonable(result)
