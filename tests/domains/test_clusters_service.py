...
class FakeClusterRepository:
    ...
    async def get_cluster_by_uid(self, cluster_uid):
        ...
        return self.cluster_row if str(cluster_uid) == self.cluster_row["cluster_uid"] else None
...
@pytest.fixture
def cluster_repository(sample_cluster_row, sample_cluster_article_rows, sample_processed_article_rows):
    processed_articles = {row["id"]: row for row in sample_processed_article_rows}
    return FakeClusterRepository(sample_cluster_row, sample_cluster_article_rows, processed_articles)


@pytest.mark.anyio
async def test_cluster_service_returns_cluster_detail(cluster_repository, sample_cluster_detail_payload):
    service = ClustersService(cluster_repository)

    result = await service.get_cluster_detail(sample_cluster_detail_payload["clusterId"])
