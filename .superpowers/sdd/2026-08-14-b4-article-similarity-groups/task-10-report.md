...
- `app/domains/pages/assembler.py` now consumes persisted snapshot grouping
  ...
  read time.
- `tests/integration/test_article_grouping_mock_pipeline.py` runs the grouping
  step through `OllamaEmbeddingProvider` and an in-process
  `httpx.MockTransport`: same-event pair, direction hard-negative, singleton,
  and forced provider failure in another cluster. This is explicitly
  process-local mock-pipeline/assembler evidence; its in-memory repository is
  not PostgreSQL persistence evidence.
- `tests/integration/test_article_grouping_postgres.py` runs the production
  `GroupSimilarArticlesStep` with a real async SQLAlchemy session,
  `ClusterRepository`, `ArticleGroupRepository`, and `BatchJobRepository`.
  It verifies SQL persistence, cluster-local `UNAVAILABLE` singleton fallback,
  exact counts, unchanged `READY` page status, restart/new-session reads, and
  vector-free PostgreSQL schema/data boundaries.
...
| `UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/contracts/test_openapi_read_routes.py tests/domains/test_page_assembler.py tests/integration/test_article_grouping_mock_pipeline.py -q` | 62 passed, 1 warning |
| `UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/contracts/test_openapi_read_routes.py tests/batch/test_group_similar_articles_step.py tests/domains/test_cluster_assembler.py tests/domains/test_page_assembler.py tests/api/test_clusters.py tests/repositories/test_article_group_repo.py tests/repositories/test_page_snapshot_write_repo.py tests/integration/test_article_grouping_mock_pipeline.py tests/integration/test_article_grouping_postgres.py -q` | 220 passed, 1 skipped, 1 warning |
| `UV_CACHE_DIR=/tmp/uv-cache uv run pytest -q` | 1,219 passed, 25 skipped, 1 warning |
...
| `STOCKAPP_MIGRATION_TEST_DSN=... UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/integration/test_article_grouping_postgres.py tests/db/test_migrations_postgresql.py -q` with ephemeral PostgreSQL 17 | 23 passed, 2 warnings |
| `UV_CACHE_DIR=/tmp/uv-cache uv run ruff check .` | clean |
| `UV_CACHE_DIR=/tmp/uv-cache uv run ruff format --check .` | 230 files already formatted |
...
the migration and delegated live integration modules, and removed it with an
EXIT trap. A final `docker ps -a --filter name=stockapp-task10-postgres`
returned no rows.
