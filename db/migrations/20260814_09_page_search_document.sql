...
BEGIN
    ...
    FOR codepoint_text IN SELECT regexp_split_to_table(value, '') LOOP
        result := result || COALESCE(
            casefold_map ->> ascii(codepoint_text)::TEXT,
            lower(codepoint_text)
        );
    END LOOP;
    RETURN result;
END;
...
CREATE INDEX IF NOT EXISTS idx_market_daily_page_search_document
    ON stock.market_daily_page USING GIN (search_document gin_trgm_ops);
